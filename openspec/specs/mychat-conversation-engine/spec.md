...
Hệ thống SHALL định nghĩa `ConversationEvent` là union gồm `send_text`, `send_image`, `select_option`, `confirm`, `submit_form`, `retry`, mang đủ dữ liệu để Conversation Engine build `ChatRequest` gửi cho `ChatAdapter`.
...
Hệ thống SHALL cung cấp hook `useConversation({ adapter, initialMessages? })` trả về `{ messages, isSending, send, sendImage, selectOption, confirm, submitForm, retry }`. Conversation Engine MUST NOT import bất kỳ module nào liên quan tới React Navigation/Expo Router, hoặc bất kỳ tên miền business (Product/Order/...) nào.
